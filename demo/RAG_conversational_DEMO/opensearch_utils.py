...
class OpenSearchUtils:
    ...
    def init_ml_settings(self):
        ...
        try:
            ...
            embedding_base_domain =  urlparse(Config.CSS_EMBEDDING_OPENAI_ENDPOINT).netloc
            embedding_escaped_url = re.sub(r'\.', r'\\.', embedding_base_domain)
            inferencing_base_domain =  urlparse(Config.CSS_OPENAI_ENDPOINT).netloc
    ...
    def register_embedding_model(self, connector_id):

        # Regsigter and deploy embedding model 
        body = {
                    "name": Config.CSS_EMBEDDING_MODEL ,
                    "description": "This is the Externally hosted CML model",
...
# Step 3: Insert Embeddings and Text into OpenSearch
    ...
    def undeploy_and_delete_model(self):
        ...
        try:
            ...
            if results == "DEPLOYED": 
                ...
                print(f"Embedding Model {self.embedding_model_id} Undeployed with")
    ...
    def delete_connector(self):
        ...
        try:
            connector_id = self.get_existing_connector(Config.CSS_CONNECTOR_NAME_EMBEDDING)
            if connector_id:
                response = self.client.http.delete(f'/_plugins/_ml/connectors/{connector_id}')
                print(f"Connector for Embedding {connector_id} DELETED")
