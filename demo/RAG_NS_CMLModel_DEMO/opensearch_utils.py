...
class OpenSearchUtils:
    ...
    def register_connector(self):
        ...
        connector_id = self.get_existing_connector(Config.CSS_CONNECTOR_NAME)
        if connector_id:
            return connector_id
        
        connector_body = {
            "name": Config.CSS_CONNECTOR_NAME,
            "description": "The connector OpenAI model service ",
            "version": 1,
            "protocol": "http",
            "parameters": {
                "endpoint": Config.CSS_EMBEDDING_OPENAI_ENDPOINT,
                "model": Config.CSS_EMBEDDING_OPENAI_MODEL,
                "api_version": Config.CSS_EMBEDDING_OPENAI_VERSION
            },
            "credential": {
                "openAI_key": Config.CSS_EMBEDDING_OPENAI_KEY
            },
            "actions": [
                {
                    "action_type": "predict",
    ...
    def registerModel(self, connector_id):
